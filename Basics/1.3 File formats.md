...
1. IHDR chunk → always the first chunk to appear in a PNG and it consists of the file width, height, CRC (cyclic redundancy checksum). The CRC is important as it contains the true "filesize" of the file, where you can run a script to bruteforce if the challenge gives you a broken / incomplete png file.
...
### Joint Photographic Experts Group (JPEGs / JPGs)
...
Unlike PNGs, JPEG's do not have chunks, rather 90% of all challenges would involve fixing the file header. Other stego involving jpg's would include the 
